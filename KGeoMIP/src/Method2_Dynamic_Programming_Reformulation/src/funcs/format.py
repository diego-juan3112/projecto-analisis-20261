...
def fmt_k_particion(grupos: list[list[tuple[int, int]]]) -> str:
    """Notación de paréntesis grandes para una k-partición de vértices.

    Extiende a k grupos la notación usada en el Excel de pruebas para
    biparticiones: una línea superior con el alcance/purview de cada grupo
    entre ⎛ ⎞ y una inferior con el mecanismo entre ⎝ ⎠, grupos contiguos.
    Los grupos se ordenan por tamaño y luego por contenido para una salida
    determinista (el grupo más pequeño primero, como en las hojas existentes).

    >>> fmt_k_particion([[(1, 8)], [(0, 0), (1, 0)]])
    '⎛ I ⎞⎛ A ⎞\\n⎝ ∅ ⎠⎝ a ⎠'
    """
    ordenados = sorted(
        ([tuple(v) for v in grupo] for grupo in grupos),
        key=lambda g: (len(g), sorted(g)),
    )
    arriba, abajo = [], []
    for grupo in ordenados:
        grupo.sort(key=lambda x: x[1])
        purv = [ABECEDARY[idx] for tiempo, idx in grupo if tiempo]
        mech = [LOWER_ABECEDARY[idx] for tiempo, idx in grupo if not tiempo]
        str_purv = ",".join(purv) if purv else VOID_STR
        str_mech = ",".join(mech) if mech else VOID_STR
        ancho = max(len(str_purv), len(str_mech)) + 2
        arriba.append(f"⎛{str_purv:^{ancho}}⎞")
        abajo.append(f"⎝{str_mech:^{ancho}}⎠")
    return "".join(arriba) + "\n" + "".join(abajo)
